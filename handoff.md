...
| Shared constants/config package | ❌ Not done | `packages/shared-config` was never created |
| ESLint + Prettier config | ✅ Done | Root configs |
| Husky pre-commit hooks | ❌ Not done | No husky dependency or `.husky/` directory |
...
| CI workflow | ⚠️ Partial | GitHub Actions — typecheck works, but lint scripts are stubs (`echo lint-ok`) |
...
| Event protocol | ❌ Not done | transcript.update, ai.response, session.*, device.*, etc. |
...
26. **Define event protocol** — transcript.update, ai.response, session.*, device.*, notification, upload.complete
