...
function renderMarkdown(text: string): string {
  return text
    .replace(
      /```(\w*)\n([\s\S]*?)```/g,
      '<pre class="bg-zinc-100 dark:bg-zinc-800 rounded-md p-3 my-2 overflow-x-auto text-sm"><code>$2</code></pre>',
    )
    .replace(
      /`([^`]+)`/g,
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'assistant',
      content:
        "Hello! I'm your Echo assistant. I can help answer questions based on the current session context.",
      timestamp: new Date(),
    },
  ]);
      ...
      <div className="flex-1 overflow-y-auto space-y-4 p-4">
        {[...messages].reverse().map((msg) => (
          ...
          >
            ...
            >
              ...
              />
              <p className="mt-1 text-xs opacity-50">{msg.timestamp.toLocaleTimeString()}</p>
            </div>
          </div>
        ))}
        ...
        {isLoading && (
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-lg bg-zinc-100 dark:bg-zinc-800 px-4 py-2">
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800 p-4">
          ...
          {buildContextChips(currentSession).map((chip) => (
            <span
              ...
              className={cn(
                ...
                chip.active ? 'bg-indigo-600/20 text-indigo-400' : 'bg-zinc-800 text-zinc-500',
              )}
            >
        ...
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
            rows={2}
            disabled={isLoading}
          />
