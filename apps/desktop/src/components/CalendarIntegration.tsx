...
export default function CalendarIntegration() {
  ...
  const handleConnect = (provider: 'google' | 'outlook') => {
    setConnections((prev) =>
      prev.map((c) =>
        c.provider === provider ? { ...c, connected: true, email: `user@${provider}.com` } : c,
      ),
    );
  };
  ...
  const handleDisconnect = (provider: 'google' | 'outlook') => {
    setConnections((prev) =>
      prev.map((c) => (c.provider === provider ? { ...c, connected: false, email: undefined } : c)),
    );
  };
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          {connections.map((conn) => (
            <div
              key={conn.provider}
              className="flex items-center justify-between rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/50 p-4"
            >
        ...
        <div className="space-y-3 rounded-lg bg-zinc-50/50 dark:bg-zinc-800/50 p-4">
          <div className="flex items-center justify-between">
            <div>
              <Label>Auto-detect Meetings</Label>
              <p className="text-xs text-zinc-500">
                Automatically detect and log calendar meetings
              </p>
            </div>
